...
function Analysis({
  ...
  const [tmdeToEdit, setTmdeToEdit] = useState(null); // State for editing TMDE
  ...
  // Handler to open the local TMDE modal for editing
  ...
  const calculateRiskMetrics = useCallback(() => {
    ...
    // 3. Unit & Uncertainty Data Preparation
    ...
    // 4. Prepare UUT Breakdown for TAR
    ...
    // 5. Prepare TMDE Breakdown for TAR
      ...
      {analysisMode === "uncertaintyTool" && (
        <div className="analysis-dashboard">
          <AddTmdeModal
            ...
            onClose={() => {
              setAddTmdeModalOpen(false);
              setTmdeToEdit(null); // Clear editing state on close
            }}
            onSave={handleSaveTmde}
            testPointData={testPointData}
            initialTmdeData={tmdeToEdit} // Pass the data to be edited
          />
